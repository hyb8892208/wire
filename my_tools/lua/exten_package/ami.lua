...
do
  ...
  local wait_for_command_reply = function(self,action_id,flag)
    assert(type(self) == "table","self is not a table (AMI object)")
    ...
    while true do
      response, err = conn:command_reply(flag) 
	  if not response then
		return nil
	  end
      if response ~= nil and action_id == response.ActionID then
        break
      end
    end
  ...
  do
    originate = function(self, phone_number)
      assert(type(self) == "table", "self is not a table (AMI object)")
      assert(type(phone_number) == "string", "phone_number is not a string")
  ...
  local command
  do
    command = function(self,ip,command_str,field,comm_type,flag)
    assert(type(self) == "table", "self is not a table (AMI object)")
    ...
    local action_id = generate_action_id(self)
    local response
    local command_type = comm_type or "Command"
    response, err = conn:command(
          command_type,
          {
            Command = command_str;
      ...
      if response then
              local response, err = self:wait_for_command_reply(action_id,flag)
      	      if response and response["text"] and not field then
                      if response["text"][1] and string.find(response["text"][1],"No such command") then
                              local err_msg = string.match(response["text"][1],"^(.*)%(.*%)")
                	      return nil,err_msg
            	      end
               end
      
               if response and field then
                     return check_reply(response,field)
               end
               return response
      end
      return nil, err
  ...
  make_ami_manager = function(config)
    ...
    return
    {
      ...
      wait_for_command_reply = wait_for_command_reply;
