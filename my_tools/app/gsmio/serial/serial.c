...
******************************************************************************/
sint32 _open_serial(char *serial_file)
{
	int fd = -1;
...
// break when receive '\n' in stream
sint32 serial_readn(uint32 serial_fd, uint8 *buf, uint32 len, uint32 timeout)
{
	sint32 ret = 0;
	uint32 readn = 0;
	fd_set fds;
	...
	to.tv_sec = timeout;
	to.tv_usec = 0;//;600000; //10000; //50000;//timeout * 1000000; // change to 100000 for pty
...
// use with mcu serial
sint32 serial_wr_(uint32 serial_fd, uint8 *buf_w, uint32 len_w, uint8 *buf_r, uint32 len_r, uint32 timeout)
{
	int ret = 0;
	...
	return serial_readn(serial_fd, buf_r, len_r, timeout);
}
/******************************************************************************
...
sint32 serial_wr(uint32 serial_fd, uint8 *buf_w, uint32 len_w, uint8 *buf_r, uint32 len_r, uint32 timeout)
{
	...
	if (ret != 4)
	{
		//zprintf("[ERRO]serial_wr:2  read[len_r:%d, ret:%d] from serial error(%d:%s) .........", 4, ret, errno, strerror(errno));
		return ret;
	}
	...
	if (len > 0)
	{
		...
		if (ret != len)
		{
			//zprintf("[ERRO]serial_wr:3  read[len_r:%d, ret:%d] from serial error(%d:%s) .........", len, ret, errno, strerror(errno));
		}
...
sint32 serial_wr_atr(uint32 serial_fd, uint8 *buf_w, uint32 len_w, uint8 *buf_r, uint32 len_r, uint32 timeout)
{
	...
	if (ret != len_w)
	{
		//zprintf("[ERRO]serial_wr_atr:1  write[len_w:%d, ret:%d] to serial error(%d:%s) .........", len_w, ret, errno, strerror(errno));
		return -1;
	}
	// read header from serial
	...
	if (ret != 4)
	{
		//zprintf("[ERRO]serial_wr_atr:2  read[len_r:%d, ret:%d] from serial error(%d:%s) .........", 4, ret, errno, strerror(errno));
		return ret;
	}
	// read body from serial
	...
	if (len > 0)
	{
		...
		if (ret != len)
		{
			//zprintf("[ERRO]serial_wr_atr:3  read[len_r:%d, ret:%d] from serial error(%d:%s) .........", len, ret, errno, strerror(errno));
		}
		ret += 4;
	}
	return ret;
...
// timeout: ms
sint32 serial_r_mcu(uint32 serial_fd, uint8 *buf, uint32 len, uint32 timeout)
{
	sint32 ret = 0;
	uint32 readn = 0;
	fd_set fds;
	...
	to.tv_sec = timeout / 1000; //0;
	to.tv_usec = (timeout % 1000) *1000; //600000; //10000; //50000;//timeout * 1000000; // change to 100000 for pty
...
// timeout: ms
sint32 serial_r_gsm(uint32 serial_fd, uint8 *buf, uint32 len, uint32 timeout)
{
	sint32 ret = 0;
	uint32 readn = 0;
	fd_set fds;
...
sint32 serial_w_mcu(uint32 serial_fd, uint8 *buf_w, uint32 len_w, uint32 timeout)
{
        if (serial_fd < 0 || buf_w == NULL)
        {
                return -1;
        }
		//zsys_debug("serial_w_mcu:  [PC --> MCU] [%s]", buf_w);
        return serial_write_n(serial_fd, buf_w, len_w, timeout);
}

sint32 serial_wr_mcu(uint32 serial_fd, uint8 *buf_w, uint32 len_w, uint8 *buf_r, uint32 len_r, uint32 timeout)
{
	int ret = -1;
	//int len = 0;

	//zsys_debug("serial_wr_mcu: [PC --> MCU] [%s]", buf_w);

	if (serial_fd < 0 || buf_w == NULL || buf_r == NULL)
	{
		zsys_error("serial_wr_mcu: args invalid");
		return -1;
	}
	ret = serial_write_n(serial_fd, buf_w, len_w, timeout);
	if (ret != len_w)
	{
		zsys_error("serial_wr_mcu: serial_write_n error(%d:%s)", errno, strerror(errno));
		return -1;
	}
	
	ret = serial_r_mcu(serial_fd, buf_r, len_r, timeout);
	//zsys_debug("serial_wr_mcu: [PC <-- MCU] [%s]", buf_r);
	return ret;
}
sint32 serial_wr_gsm(uint32 serial_fd, uint8 *buf_w, uint32 len_w, uint8 *buf_r, uint32 len_r, uint32 timeout)
{
	int ret = -1;
	//int len = 0;
	...
	ret = serial_r_gsm(serial_fd, buf_r, len_r, timeout);
	//zsys_debug("serial_wr_mcu: [PC <-- MCU] [%s]", buf_r);
	return ret;
}
