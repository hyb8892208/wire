...
struct NODE_CALLBACK *cb_node_new(char *name)
{
    ...
    node_new = (struct NODE_CALLBACK *)malloc(sizeof(struct NODE_CALLBACK));
    if ( node_new == NULL )
        return NULL;
    ...
    len = strlen(name);
    node_new->name = (char *)malloc(len+1);
...
{
    /* 头节点初始化 */
    g_cli_head = cb_node_new((char *)"head");
    if ( g_cli_head == NULL )
    {
    ...
    loadHistoryCmd((char *)HISTORY_FILE_PATH); /*加载历史命令*/
}
