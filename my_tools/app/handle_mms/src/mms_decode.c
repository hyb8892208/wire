...
static int MMSEncStringFrom(const unsigned char *data, const int dim, int offset, char **strval){
	int field;
	int length;
	int	count, i;
	field = data[offset];
	...
	if (field < 0x20) {
		if(data[offset+3] > 0x39){//means char-set
			offset+=2;
			length = MMSValueLength(data, dim, offset, &count);
			...
			*strval = malloc(length);
			if (length < 2) {
				**strval = '\0';
				...
				for (i=0; i!=length-1; i++) {
					//printf("%02x ",data[offset+count+1+i]);
					if(data[offset+count+1+i] < 0x30)
						break;
					if(data[offset+count+1+i] > 0x39)
						break;
					(*strval)[i] = data[offset+count+1+i];
				}
				(*strval)[i] = '\0';
...
static int MMSHeader(mms_message *msg, const unsigned char *data, const int dim, unsigned int *ctype)
{
    ...
    while (cont && offset < dim) {
      ...
      //          MMSEncString(data, dim, offset, &str);
      		  MMSEncStringFrom(data, dim, offset, &str);
...
static int MMsBody(mms_message *msg, const unsigned char *data, const int dim, int offset, unsigned int ctype)
{
    int nparts, i;
    int count;
    int header_len, data_len,act_len;
    unsigned int tmp;
    FILE *fp;
    ...
    for (i=0; i!=nparts; i++) {
        ...
        //printf("Ctype: %s\n", msg->part[i].ctype);
	if(strcmp(msg->part[i].ctype, "text/plain") == 0){
			msg->content = (char *)malloc(data_len+1);
			strncpy(msg->content, data+offset+header_len, data_len);
			msg->content[data_len] = '\0';
			//act_len = data_len > contentlen ? contentlen:data_len;
			//strncpy(content, data+offset+header_len, act_len);
			//content[act_len] = '\0';
			break;
	}
...
int MMSDecode(mms_message *msg, const unsigned char *data, const int len)
{
    int offset;
    unsigned int ctype;
    ...
    if ((ctype == 0x22) || (ctype == 0x23) || (ctype == 0x24) ||
        ...
        MMsBody(msg, data, len, offset, ctype);
    }
...
//#define MMS_DECODE_TEST
#ifdef MMS_DECODE_TEST
#define MMS_RAW_DIM 10240
