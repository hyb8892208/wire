...
sint32 serial_wr(uint32 serial_fd, uint8 *buf_w, uint32 len_w, uint8 *buf_r, uint32 len_r, uint32 timeout)
{
	...
	if (ret != len_w)
	{
		zprintf(ERROR,"[ERRO]serial_wr:1  write[len_w:%d, ret:%d] to serial error(%d:%s) .........", len_w, ret, errno, strerror(errno));
		return -1;
	}
	...
	if (ret != 4)
	{
		zprintf(ERROR,"[ERRO]serial_wr:2  read[len_r:%d, ret:%d] from serial error(%d:%s) .........", 4, ret, errno, strerror(errno));
		return ret;
	}
	...
	if (len > 0)
	{
		...
		if (ret != len)
		{
			zprintf(ERROR,"[ERRO]serial_wr:3  read[len_r:%d, ret:%d] from serial error(%d:%s) .........", len, ret, errno, strerror(errno));
		}
...
sint32 serial_wr_atr(uint32 serial_fd, uint8 *buf_w, uint32 len_w, uint8 *buf_r, uint32 len_r, uint32 timeout)
{
	...
	if (ret != len_w)
	{
		zprintf(ERROR,"[ERRO]serial_wr_atr:1  write[len_w:%d, ret:%d] to serial error(%d:%s) .........", len_w, ret, errno, strerror(errno));
		return -1;
	}
	// read header from serial
	...
	if (ret != 4)
	{
		zprintf(ERROR,"[ERRO]serial_wr_atr:2  read[len_r:%d, ret:%d] from serial error(%d:%s) .........", 4, ret, errno, strerror(errno));
		return ret;
	}
	// read body from serial
	...
	if (len > 0)
	{
		...
		if (ret != len)
		{
			zprintf(ERROR,"[ERRO]serial_wr_atr:3  read[len_r:%d, ret:%d] from serial error(%d:%s) .........", len, ret, errno, strerror(errno));
		}
		ret += 4;
	}
	return ret;
