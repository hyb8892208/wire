...
void init_simcard_enable(void)
{
	...
	while(ret != HDL_OK){
		...
		zsys_debug("debug: init_simcard_enable buf is %s", buf);
		for (i = 0; i < strlen(buf); i++){
        	if (buf[i] != '0') {
				ret = HDL_OK;
				zsys_error("debug: init_simcard_enable: detect setting port(%d) successful", ALL_CHN);
				break;
			}
    	}
		j++;
		if (j >= retry ) {
			zsys_error("error: init_simcard_enable: port(%d) failed,retry failed(%d),jump out", ALL_CHN, i);
			break;
		}
	}
...
void init_hw_version(void)
{
	...
	char *filename = "/tmp/hardware_version";
    char *p;
	FILE *fd;
