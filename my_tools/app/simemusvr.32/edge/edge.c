...
#define N2N_EDGE_MISS_ACK_MAX	5		/* Number of failed miss register ack */
...
struct n2n_edge
{
	...
	n2n_password_t		password;
	n2n_privateip_t		private_ip;
...
static unsigned char *encryoptpasswd(unsigned char *passwd,unsigned char *md5)
{
	MD5_CTX ctx;
	MD5_Init(&ctx);
	MD5_Update(&ctx, (const void *)passwd,strlen((const char *)passwd));
	MD5_Final(md5, &ctx);
	
	return md5;
}

static void echo_status(int stat)
{
	static int flag = 0;
	if(0 == (flag + stat)){
		system("echo 'connect success ' > /tmp/OPlink.status");
		flag = 1;
	}else if(2 == (flag + stat)){
		system("echo 'connect failed ' > /tmp/OPlink.status");
		flag = 0;
	}
}
                                 ...
                                 const n2n_sock_t * supernode)
{
    ...
    cmn.flags = 0;
    memcpy( cmn.community, eee->community_name, N2N_COMMUNITY_SIZE );
	memcpy(cmn.password,eee->password,N2N_PASSWORD_SIZE);
	memcpy(cmn.private_ip,eee->private_ip,N2N_PRIVATEIP_SIZE);
...
static void update_supernode_reg( n2n_edge_t * eee, time_t nowTime )
{
    ...
    if ( 0 == eee->sup_attempts )
    {
		...
		echo_status(1);
		g_link_num++;
		if(g_link_num > N2N_EDGE_MISS_ACK_MAX){
			traceEvent(TRACE_ERROR, "node miss,will exit!!!");
			exit(-1);
		}
...
static int get_ethernet_mac(char *device,char *mac_addr)
{
    struct ifreq ifreq;
    int sock;
	...
	traceEvent( TRACE_DEBUG, "MAC:%s",mac_addr );
	return 0;
}
static int set_localip(n2n_edge_t * eee) {
    int     local_port;
...
void hide_arg(int argc,char *argv[])
{
	int i;
	for (i = 1; i < argc; i++)
	{
...
int real_main(int argc, char* argv[])
{
    ...
    char    tuntap_dev_name[N2N_IFNAMSIZ] = "OPlink";
    ...
    char	encrypt_key[N2N_PASSWORD_SIZE] = {0};
	...
	get_ethernet_mac("eth0",device_mac);
	strncpy( (char *)eee.private_ip, ip_addr, N2N_PRIVATEIP_SIZE);
    /* {int k;for(k=0;k<effectiveargc;++k)  printf("%s\n",effectiveargv[k]);} */

    optarg = NULL;
    while((opt = getopt_long(effectiveargc,
                             effectiveargv,
                             "a:bc:Eu:g:m:M:s:d:l:L:i:p:fvhrt:R", long_options, NULL)) != EOF)
    {
        switch (opt)
        {        
        ...
        case 'c': /* community as a string */
        {
			...
			encryoptpasswd((unsigned char *)eee.community_name,(unsigned char *)eee.password);
			memcpy(encrypt_key,eee.password,N2N_PASSWORD_SIZE);
            break;
        }
    ...
    if ( ('\0' == encrypt_key[0] ) && ( 0 == strlen(eee.keyschedule)) )
    {
    ...
    if ( '\0' != encrypt_key[0] ) {
        if(edge_init_twofish( &eee, (uint8_t *)(encrypt_key), N2N_PASSWORD_SIZE) < 0) {
