...
#define ACTIONID_LABEL         "ActionID: callmonitor-"
...
//	printf("readbuff=%s\n", readbuff);
	...
	while(p) {
		...
		if(call_status == SIM_OUT){
		    call_status = check_sim_out(readbuff);
		}else if(call_status == SMS_SUCCESS){
		    call_status = check_sms_succesfully(readbuff);
		}else if(call_status == SMS_REPORT_SUCCESS){
		    call_status = check_sms_report_succesfully(readbuff);
		}else if(call_status == CALL_HANGUP){
//		    set_lastcall_state(readbuff);
...
static int event_sock_recreate(void)
{
	...
	printf("event sock rereate success\n");
	sleep(2);
	return sockfd;
}
