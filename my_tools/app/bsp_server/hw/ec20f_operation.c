...
/***************************************************
文件描述  : 实现控制、检测ec20f状态接口, 包括：
            1) set SIM card enable/disable
            2) get SIM card enable/disable status
            3) ec20f模块上断/掉电(VBAT ON/OFF)
            4) ec20f模块紧急开机/关机(EMERGENCY ON/OFF)
            5) ec20f模块按键开机/关机(POWR_KEY ON/OFF)
            6) SIM card insert status
            7) 通道模块板(包含2块板的模块）插拔状态
            8) ec20f模块开机状态
修改/时间 : wengang.mu@openvox.cn/2018.03.28
*****************************************************/
#include <unistd.h>
...
/*************************************************
  ...
  函数返回 :  0 -- 成功
             <0 -- 失败
  备注     ： 函数退出不确保模块已经重启完成
  修改/时间 : wengang.mu@openvox.cn/2018.01.29
*************************************************/
static int ec20f_reboot(struct swg_mod_brd *brd, int chan)
{
    int reg, bit, res, i;
...
/***********************************************
*设置vbat供电断电状态,0供电，1断电
*输入参数：mb----模块板结构体指针
*          md_idx----模块板上的通道索引号
*输出参数：reg_value_l----vbat低寄存器值
*          reg_value_h----vbat高寄存器值
***********************************************/
int ec20f_translate_vbat_v010000(struct swg_mod_brd *mb, int mb_idx, int *reg_l, int *reg_h){
...
/***********************************************
*设置vbat供电断电状态，1供电，0断电
*输入参数：mb----模块板结构体指针
*          md_idx----模块板上的通道索引号
*输出参数：reg_value_l----vbat低寄存器值
*          reg_value_h----vbat高寄存器值
***********************************************/
int ec20f_translate_vbat_v020000(struct swg_mod_brd *mb, int mb_idx, int *reg_l, int *reg_h){
	int *reg;
	unsigned char *vbat_state;
	int bit;
...
/***********************************************
*转换vbat供电断电状态,0供电，1断电
*输入参数：mb----模块板结构体指针
*          md_idx----模块板上的通道索引号
*          reg_value----一维数组,vbat寄存器值
***********************************************/
int ec20f_translate_vbat_status_v010000(struct swg_mod_brd *mb, int md_idx, int *reg_value){
...
/***********************************************
*转换vbat供电断电状态,1供电，0断电
*输入参数：mb----模块板结构体指针
*          md_idx----模块板上的通道索引号
*          reg_value----一维数组,vbat寄存器值
***********************************************/
int ec20f_translate_vbat_status_v020000(struct swg_mod_brd *mb, int md_idx, int *reg_value){
	unsigned int vbat_bit_map;
	unsigned int bit_value;
	int bit;
...
int ec20f_translate_power_status(struct swg_mod_brd *mb, int mb_idx, int *reg_value){
	unsigned int power_bit_map;
	unsigned int bit_value;
	int bit;
    /* 获取所有通道状态值 */
    /* MB_REG_POWER_L地址在MB_REG_POWER_H之后 */
    power_bit_map = ~(((reg_value[0] & 0xFF) << 8) | (reg_value[1] & 0xFF));  /* 寄存器值0表示开机，1表示关机 */
	bit_value = (power_bit_map>>mb_idx)&0x1;

	bit = mb_idx % MB_REG_WIDE;
	
	if(mb_idx/MB_REG_WIDE){
		mb->power_status_h &= ~(1<<bit);//清空vbat_lamp中对应bit位
	    mb->power_status_h |= (bit_value << bit);//
	}else{
		mb->power_status_l &= ~(1<<bit);//清空vbat_lamp中对应bit位
...
{
    /* ec20f 没有紧急关机，与之对应的是重启 */
    .emergency_power_off            = ec20f_reboot, 
    .translate_vbat                  = ec20f_translate_vbat_v010000,
    .translate_vbat_status           = ec20f_translate_vbat_status_v010000,
    .translate_power_status          = ec20f_translate_power_status,
};
...
struct sim_card_ops sim_ec20f_ops_v020000 =
{
    /* ec20f 没有紧急关机，与之对应的是重启 */
    .emergency_power_off            = ec20f_reboot, 
    .translate_vbat                  = ec20f_translate_vbat_v020000,
    .translate_vbat_status           = ec20f_translate_vbat_status_v010000,
    .translate_power_status          = ec20f_translate_power_status,
};
