...
<?php
$smsoutbox_file = '/data/log/smsoutbox.db';
$line_counts = 10;

if(!file_exists($smsoutbox_file)) {
...
if($_POST) {
	if(isset($_POST['send'])) {
		if($_POST['send'] == 'Delete') {
			...
			save_user_record("","SMS->SMS Outbox:Delete SMS");
			...
			save_user_record("","SMS->SMS Outbox:Delete All SMS");
...
if($cluster_info['mode'] != 'master' && $filter_sql == "") {
	$filter_sql = "where (port='1' OR port='".$__BRD_HEAD__."1-".$__GSM_HEAD__."1'"; //use this format
	for ($i = 2; $i <= $__GSM_SUM__; $i++) {
		$filter_sql .= " OR port='$i' OR port='".$__BRD_HEAD__."1-".$__GSM_HEAD__."$i'";
	}
	$filter_sql .= ")";

	/*
	$filter_sql  = "where (port='1' OR port='2' OR port='3' OR port='4'";
	$filter_sql .= " OR port='".$__BRD_HEAD__."1-".$__GSM_HEAD__."1'";
	$filter_sql .= " OR port='".$__BRD_HEAD__."1-".$__GSM_HEAD__."2'";
	$filter_sql .= " OR port='".$__BRD_HEAD__."1-".$__GSM_HEAD__."3'";
	$filter_sql .= " OR port='".$__BRD_HEAD__."1-".$__GSM_HEAD__."4'";
	$filter_sql .= ")";
	 */
}
...
if(isset($_GET['start_datetime_filter'])){
	...
	$start_datetime_filter = date("Y-m-d H:i:s",strtotime(str_replace('/','-',$start_datetime_filter)));
}else if(isset($_POST['start_datetime_filter'])){
	$start_datetime_filter = $_POST['start_datetime_filter'];
}

$start_datetime_filter = str_replace("/", "-", $start_datetime_filter);
...
if(isset($_GET['end_datetime_filter'])){
	...
	$end_datetime_filter = date("Y-m-d H:i:s",strtotime(str_replace('/','-',$end_datetime_filter)));
}else if(isset($_POST['end_datetime_filter'])){
	$end_datetime_filter = $_POST['end_datetime_filter'];
}

$end_datetime_filter = str_replace("/", "-", $end_datetime_filter);
...
$tmp = $db->try_query("select count(*) from sms_out $filter_sql $sort_sql");
if($tmp === false){
	$tmp = "";
	$counts = 0;
}else{
...
function del_sms($sms_array)
{
	...
	if($str != "") {
		$str = rtrim($str,"or ");
		$hlock = lock_file($smsoutbox_file);
		$db->try_query("delete from sms_out where $str");
		unlock_file($hlock);
	}
}
...
function export_excel()
{
	...
	if(isset($item[5])) {
		...
		$minute = $item[4];
		$second = $item[5];
		$output_name = "smsoutbox-$year-$month-$date-$hour-$minute-$second.txt";
	}
	...
	$hlock=lock_file($smsoutbox_file);
	$results = $db->try_query("select * from sms_out $filter_sql $sort_sql");
	unlock_file($hlock);
	...
	while($res = @$results->fetchArray()) {
		...
		echo $res['port']."\t".$res['phonenumber']."\t".$res['time']."\t".$res['status']."\t".$res['message']."\n";
	}
	if($results === false){
...
?>
	...
	$(function() {
		$( "#start_datetime_filter" ).datetimepicker({dateFormat: "dd/mm/yy", timeFormat: "HH:mm:ss"});
		$( "#end_datetime_filter" ).datetimepicker({dateFormat: "dd/mm/yy", timeFormat: "HH:mm:ss"});
	...
	<!-- Filter table -->
<?php
	$filter_target = "where (port='1' OR port='".$__BRD_HEAD__."1-".$__GSM_HEAD__."1'"; //use this format
	for ($i = 2; $i <= $__GSM_SUM__; $i++) {
...
?>
	<div class="content">
		<table class="table_show">
			<tr>
				...
				<th width='15%'><?php echo language('Phone Number');?></th>
				<th width='25%'><?php echo language('Time');?></th>
				<th width='40%'><?php echo language('Message Keywords');?></th>
			</tr>
	...
	<div id="button_save">
		<button type="button" id="do_filter" ><?php echo language('Filter');?></button>
		<button type="button" id="clean_filter" ><?php echo language('Clean Filter');?></button>
	</div>
...
<?php
	$sql_requery="select * from sms_out $filter_sql $sort_sql limit $start_record, $line_counts";
	...
	<div class="content">
		<table class="table_show">
			<tr>
				...
				<th width='10%' class="<?php echo $port_class?>" onclick="sort_click(this,'port')"><?php echo language('Port');?></th>
				
				<?php if($phonenum_res['general']['phonenum_switch'] == 'on'){ ?>
				<th><?php echo language('Sent From');?></th>
				<?php } ?>
				
				<th width='10%' class="<?php echo $phonenumber_class?>" onclick="sort_click(this,'phonenumber')"><?php echo language('Sent To');?></th>
				<th width='15%' class="<?php echo $time_class?>" onclick="sort_click(this,'time')"><?php echo language('Time');?></th>
			...
			<tr>
				...
				<td> 
					<?php 
						echo get_gsm_name_by_channel($res['port'],1,false);
				...
				<td> 
					<?php echo $res['time'];//echo date("d/m/Y H:i:s",strtotime($res['time']));?>
				</td>
				<td>
					<?php
					if ($res['status'] == '0') {
...
?>
	...
	<div id="button_save">
		...
		<?php if(!$only_view){ ?>
		<button type="submit" onclick="document.getElementById('send').value='Delete';return confirm('Are you sure to delete you selected ?')"><?php echo language('Delete');?></button>
		<button type="submit" onclick="document.getElementById('send').value='Clean Up';return confirm('Are you sure to delete all smss ?')"><?php echo language('Clean Up');?></button>
		<?php } ?>
		
		<button type="submit" onclick="document.getElementById('send').value='Export';"><?php echo language('Export');?></button>
	</div>
