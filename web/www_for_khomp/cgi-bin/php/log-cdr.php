	...
	if(isset($_POST['send'])) {
		if($_POST['send'] == 'Delete') {
			...
			if(isset($_POST['log'])) {
				...
				save_user_record("","LOGS->CDR:Delete Log");
			}
			...
			save_user_record("","LOGS->CDR:Delete All Log");
...
if(isset($_GET['starttime_from_filter'])){
	...
	$starttime_from_filter = date("Y-m-d H:i:s",strtotime(str_replace('/','-',$starttime_from_filter)));
...
if(isset($_GET['starttime_to_filter'])){
	...
	$starttime_to_filter = date("Y-m-d H:i:s",strtotime(str_replace('/','-',$starttime_to_filter)));
...
?>
	...
	$(function() {
		$( "#starttime_from_filter" ).datetimepicker({dateFormat: "dd-mm-yy", timeFormat: "HH:mm:ss"});
		$( "#starttime_to_filter" ).datetimepicker({dateFormat: "dd-mm-yy", timeFormat: "HH:mm:ss"});
...
?>
	...
	<div class="content">
		<table class="table_show">
			<tr>
				<th style="width:03%" lass="nosort"></th>
	...
	<div id="button_save">
		<button type="button" onclick='filter();' ><?php echo language('Filter');?></button>
		<button type="button" onclick='clear_filter();' ><?php echo language('Clean Filter');?></button>
	</div>
<?php
	...
	<div class="content">
		<table class="table_show">
			...
			<tr>
				...
				<td> 
					<?php echo date("d/m/Y H:i:s",strtotime($res['starttime']));?>
				</td>
				<td> 
...
?>
	...
	<div class="content" style="margin:bottom;">
		<span class="title gsm_outbound" style="cursor:pointer;">
	...
	<table class="table_show">
		<tr><th colspan="9"><?php echo language('GSM Outbound');?></th></tr>
		<tr>
		...
		$to_sql = "select `to`,
						...
						sum(case when result = 'NO ANSWERED' then 1 else 0 end) as no_answer,
						sum(case when result = 'BUSY' then 1 else 0 end) as busy,
						sum(case when result = 'FAILED' then 1 else 0 end) as failed,
						sum(case when result <> 'ANSWERED' and result <> 'NO ANSWER' and result <> 'BUSY' and result <> 'FAILED' then 1 else 0 end) as other
	...
	<table class="table_show">
		<tr><th colspan="9"><?php echo language('GSM Inbound');?></th></tr>
		<tr>
			<th><?php echo language('Port');?></th>
			<th><?php echo language('Sim Number');?></th>
			<th><?php echo language('All Calls');?></th>
