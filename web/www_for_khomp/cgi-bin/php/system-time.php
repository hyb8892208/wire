...
function click(){
	...
	$.ajax({
		...
		success: function(data) {
			...
			window.setInterval('window.location.href="/cgi-bin/php/system-time.php";',1000);
...
function sync_time_from_ntp() 
{
	...
	$(".ui-state-default, .ui-widget-content .ui-state-default, .ui-widget-header .ui-state-default").css("border","1px solid #fff");
	$(".ui-state-default, .ui-widget-content .ui-state-default, .ui-widget-header .ui-state-default").css("background","#fff");
...
function check()
{
	...
	if(ntp_server1 != '') {
		if(!check_domain(ntp_server1)) {
			document.getElementById('ntp_server1').focus();
	...
	if(ntp_server2 != '') {
		if(!check_domain(ntp_server2)) {
			document.getElementById('ntp_server2').focus();
	...
	if(ntp_server3 != '') {
		if(!check_domain(ntp_server3)) {
			document.getElementById('ntp_server3').focus();
...
if($_POST && isset($_POST['send'])) {
	if($_POST['send'] == 'Save Data') {
		...
		save_user_record("","SYSTEM->Time:Save");
	}
}
...
?>
	...
	<div class="content">
		<span class="title">
			<?php echo language('Time Settings');?>
			...
			<div class="tip_main">
				...
				<div class="tip_help">
					...
					<div class="tip_content">
						<?php if(is_show_language_help('System Time')){ ?>
							<b><?php echo language('System Time','System Time');?>:</b><br/>
							<?php echo language('System Time help', 'Your gateway system time.'); ?>
							
							<br/><br/>
						<?php } ?>
						
						<?php if(is_show_language_help('Time Zone')){ ?>
							<b><?php echo language('Time Zone','Time Zone');?>:</b><br/>
							<?php
								$help = 'The world time zone. Please select the one which is the same as or the closest to your city.';
								echo language('Time Zone help', $help);
							?>
							
							<br/><br/>
						<?php } ?>
						
						<?php if(is_show_language_help('POSIX TZ String')){ ?>
							<b><?php echo language('POSIX TZ String');?>:</b><br/>
							<?php echo language('POSIX TZ String help', 'Posix timezone strings.'); ?>
							
							<br/><br/>
						<?php } ?>
						
						<?php if(is_show_language_help('Auto-Sync from Server')){ ?>
							<b><?php echo language('Auto-Sync from Server');?>:</b><br/>
							<?php
						...
						<?php if(is_show_language_help('Auto-Sync Type')){ ?>
							<b><?php echo language('Auto-Sync Type');?>:</b><br/>
		...
		<div class="tab_item">
			<span>
				<?php echo language('System Time','System Time');?>:
			</span>
		...
		<div class="tab_item">
			<span>
				<?php echo language('Time Zone','Time Zone');?>:
			</span>
			<div class="tab_item_right">
				<select id="system_timezone" name="system_timezone" onchange="modechange(this)">
					...
					<optgroup label="<?php echo language('Central and South America');?>">
						...
						<option  value="America/DeNoronha@UTC-2"><?php echo language('Fernando de Noronha,Brazil');?></option>
						<option  value="America/Manaus@UTC-4"><?php echo language('Manaus,Brazil');?></option>
						<option  value="America/RioBranco@UTC-5"><?php echo language('Rio Branco,Brazil');?></option>
		...
		<div class="tab_item">
			...
			<div class="tab_item_right">
				<input id="show_TZ" type="text" name="show_TZ" readonly="readonly" value="This field requires the JavaScript support." />
			</div>
		</div>
		...
		<?php
			...
			for($i=1; $i<=3; $i++) {
				echo "<div class='tab_item'>";
				echo "<span>";
				echo language("NTP Server $i").":";
				echo "</span>";
				echo "<div class='tab_item_right'>";
				echo "<span id='cntp_server${i}'></span>";
				echo "<input id='ntp_server${i}' type='text' name='ntp_server${i}' value='$cf_ntp[$i]' />";
				echo "</div>";
				echo "</div>";
			}
		?>
		...
		<div class="tab_item">
			...
			<div class="tab_item_right">
				<span><input type="checkbox" id="auto_sw" name="auto_sw" <?php echo $auto_sw?>  /></span>
			</div>
		</div>
	...
	<?php if(!$only_view){ ?>
		<button type="submit" class="gen_long_btn" onclick="document.getElementById('send').value='Save Data';return check();" /><?php echo language('Save Data');?></button>
