...
function download_all_at_log()
{
	...
	$tar_dir = '/tmp/var/log/asterisk/ATlog';
	...
	for($i=1;$i<=$__GSM_SUM__;$i++){
		$src = '/tmp/var/log/asterisk/at/'.$i;
		$des = $tar_dir.'/'.get_gsm_name_by_channel($i,1);;
		if(file_exists('/tmp/var/log/asterisk/at/'.$i)){
			copy($src, $des);
		}else{
			touch($des);
		}
	...
	//pack file
	$pack_cmd="tar vcz -f $tar_path -C /tmp/var/log/asterisk/ ./ATlog";
	//echo $pack_cmd;
	...
	exec("rm -rf /tmp/var/log/asterisk/ATlog");
...
?>
	...
	<div class="content">
		<span class="title"><?php echo language('AT Commands Logs');?></span>
