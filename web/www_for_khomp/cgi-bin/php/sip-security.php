...
function show_security(){
	...
	if(isset($tls_para_array['tlsbindaddr'])){
		if(strstr($tls_para_array['tlsbindaddr'],"::")){
			$tlsbindaddr = explode(':', $tls_para_array['tlsbindaddr']);
			$port = $tlsbindaddr[3];
		}else{
...
<form enctype="multipart/form-data" action="<?php echo $_SERVER['PHP_SELF']?>" method="post">
	<div class="content">
			...
			<div class="tip_main">
				...
				<div class="tip_help">
					...
					<div class="tip_content">
						...
						<?php if(is_show_language_help('TLS Verify Server')){ ?>
							<b><?php echo language('TLS Verify Server');?>:</b><br/>
							<?php echo language('TLS Verify Server help','Enable or disable tls verify server(default is no).');?>
						...
						<?php if(is_show_language_help('TLS Client Method')){ ?>
							<b><?php echo language('TLS Client Method');?>:</b><br/>
		...
		<div class="tab_item">
			...
			<div class="tab_item_right">
				<span><input type="checkbox" name="tls_enable" id="tls_enable" <?php if(strcmp($tls_enable,"yes")==0) echo "checked";?> /></span>
			</div>
		</div>
		...
		<div class="tab_item">
			...
			<div class="tab_item_right">
				<span><input type="checkbox" name="tls_verify_server" id="tls_verify_server" <?php if(strcmp($tls_verify_server,"no")==0) echo "checked"; ?> /></span>
			</div>
		</div>
	...
	<div class="content">
		<span class="title"><?php echo language('TLS keys');?></span>
	...
	<div class="content">
		<span class="title"><?php echo language('Key Files');?></span>
...
function create_key(){
	...
	if($type == "server"){
		...
		if(strpos(end($output),"asterisk.pem")){
			...
			save_user_record("","VOIP->SIP Account Security:Create Server Key,keyname=".$keyname);
		...
		if(strpos(end($output),$keyname.".pem")){
			...
			save_user_record("","VOIP->SIP Account Security:Create Client Key,keyname=".$keyname);
		} else {  // failed to create key
			echo "<script type=\"text/javascript\">alert('Please checking the server keys is exist or not and client password must be same with Server!!!')</script>";
		}
	}
...
function delete_key(){
	...
	save_user_record("","VOIP->SIP Account Security:Delete Key,key=".$section);
}
...
function delete_file(){
	...
	save_user_record("","VOIP->SIP Account Security:Delete File,file=".$file);
}
...
if($_POST) {
	if (isset($_POST['send']) && $_POST['send'] == "Save") {
		...
		save_user_record("","VOIP->SIP Account Security:TLS Setting Save");
		...
		save_user_record("","VOIP->SIP Account Security:Upload Key");
