...
<form enctype="multipart/form-data" action="<?php echo get_self() ?>" method="post">
	...
	<div class="content">
		<span class="title"><?php echo language('Edit');echo " ".basename($filepath);?></span>
	...
	<div id="button_save">
		...
		<button type="button" onclick="goto()" ><?php echo language('Cancel');?></button>
		<button type="submit" onclick="document.getElementById('send').value='Delete File';return confirm('<?php echo language('Delete File confirm','Are you sure to delete this file?');?>')" ><?php echo language('Delete File');?></button>
	</div>
</form>
<?php
}
...
<form enctype="multipart/form-data" action="<?php echo get_self() ?>" method="post">
	...
	<div class="content">
		<span class="title"><?php echo language('Edit');?> <input type="text" name="newfilename" value="<?php echo $newfilename?>" style="width:120px;" />.conf</span>
...
<form enctype="multipart/form-data" action="<?php echo get_self() ?>" method="post">
	...
	<div class="content">
		<span class="title"><?php echo language('Configuration Files');?></span>
...
?>
	...
	<div id="button_save">
		<button type="submit" onclick="document.getElementById('send').value='New Configuration File';"><?php echo language('New Configuration File');?></button>
		<button type="submit" onclick="document.getElementById('send').value='Reload Asterisk';return confirm('<?php echo language('Reload Asterisk confirm','Are you sure to reload asterisk now?');?>')" ><?php echo language('Reload Asterisk');?></button>
	</div>
</form>
...
if($_POST && isset($_POST['send'])) {
	if($_POST['send'] == 'Save') {
		...
		save_user_record("","ADVANCED->Asterisk File Editor:Save,filepath=".$path);
	} else if($_POST['send'] == 'Reload Asterisk') {
		ast_reload();
		
		save_user_record("","ADVANCED->Asterisk File Editor:Reload Asterisk");
		...
		if(isset($_POST['filepath'])) {
			if(is_cfgfile($_POST['filepath'])) {
				...
				save_user_record("","ADVANCED->Asterisk File Editor:Delete,filepath=".$_POST['filepath']);
			}
		} 
	}
}
