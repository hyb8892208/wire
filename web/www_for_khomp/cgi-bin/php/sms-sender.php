...
function trace_sms($msg, $to, $span, $retry,$result, $flag = "tr") {
	if($flag == "tr"){		
		...
		echo "<table class='table_show' style='font-size:12px;'>";
...
function trace_sms_statistics($total, $success, $fail){
	...
	$table  .= "<table class='table_show'>";
	$table 	.=	"<tr style='height:26px;'>";
...
function trace_sms_error($error,$status) {
	...
	$table .= "<table class='table_show' style='font-size:12px;'>";
	$table .= "<tr>";
...
<form enctype="multipart/form-data" action="<?php echo get_self() ?>" method="post" onsubmit="return check_send()"> 
	...
	<div class="content">
				...
				<span>
					...
					<div class="tip_main">
						...
						<div class="tip_help">
							...
							<div class="tip_content">
								<b><?php echo language('Flash SMS');?>:</b><br/>
								<?php echo language('Flash SMS help','Flash SMS');?>
							</div>
						</div>
					</div>
				</span>
				<div class="tab_item_right">
					<span><input type="checkbox" id="flash_sms" name="flash_sms" style="" onchange="" <?php if($flash_sms == 1) echo "checked";?> /></span>
				</div>
			</div>
		<?php } ?>
			...
			<span>
				...
				<div class="tip_main">
					...
					<div class="tip_help">
						...
						<div class="tip_content">
							<b><?php echo language('Load numbers from text file');?>:</b><br/>
							<?php echo language('Load numbers from text file help','Load number from text file');?>
						</div>
					</div>
				</div>
			</span>
		...
		<div class="tab_item">
			...
			<div class="tab_item_right">
				<?php
				 ...
				 echo "<textarea style=\"float:right;\" id=\"dest_num\" name=\"dest_num\" width=\"100%\" rows=\"5\" cols=\"80\" >$value</textarea>";
				?>
				<div class="clear"></div>
				<h4><?php echo language('Destination Number help2',"\"; semicolon\" , \"| vertical Bar\" , \" , comma \" , \"   blank \" , \" : colon \" , \" . dot \" were treated as separators in Destination Number List");?></h4>
			</div>
			<div class="clear"></div>
		</div>
			...
			<span>
				...
				<div class="tip_main">
					...
					<div class="tip_help">
						...
						<div class="tip_content">
							<b><?php echo language('Message');?>:</b><br/>
		...
		<div id="button_save">
			...
			<?php if(!$only_view){ ?>
			<button type="submit" onclick="cookie_update();document.getElementById('send').value='Send';"><?php echo language('Send');?></button>
			<button type="submit" onclick="document.getElementById('send').value='Stop';" ><?php echo language('Stop');?></button>
			<?php } ?>
...
<?php
	if ($_STOP){
		...
		save_user_record("","SMS->SMS Sender:Stop");
	}
	if($_SEND) {
		
		save_user_record("","SMS->SMS Sender:Send");
