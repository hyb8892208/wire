	...
	if(isset($_POST['send'])) {
		if($_POST['send'] == 'Delete') {
			...
			if(isset($_POST['sms'])) {
				...
				save_user_record("","SMS->SMS Inbox:Delete SMS");
			}
			...
			save_user_record("","SMS->SMS Inbox:Delete All SMS");
...
function export_excel()
{
	...
	while($res = @$results->fetchArray()) {
		...
		echo $res['port']."\t".$res['phonenumber']."\t".$res['time']."\t".$res['message']."\n";
	}
	if($results === false){
	...
	<div class="content">
		<table class="table_show">
			<tr>
				...
				<th width='15%' class="<?php echo $port_class?>" onclick="sort_click(this,'port')"><?php echo language('Port');?></th>
				
				<?php if($phonenum_res['general']['phonenum_switch'] == 'on'){ ?>
				<th><?php echo language('Mobile Number');?></th>
				<?php } ?>
				
				<th width='15%' class="<?php echo $phonenumber_class?>" onclick="sort_click(this,'phonenumber')"><?php echo language('Phone Number');?></th>
			...
			<?php
				...
				<tr>
					...
					<td> 
						<?php echo date("d/m/Y H:i:s",strtotime($res['time'])); ?>
					</td>
					<td> 
