...
function saveChangePassword()
{
	...
	$web_server_array = $aql->query("select * from web_server.conf");

	if(!isset($web_server_array)){
		$aql->assign_addsection('general', '');
	}

	if(isset($_POST['new_username']) && isset($_POST['new_password']) && isset($_POST['confirm_password']) && trim($_POST['new_username']) != '' && trim($_POST['new_password']) != ''){
		if(trim($_POST['new_password']) == trim($_POST['confirm_password'])){
			if(isset($web_server_array['general']['username'])) {
				$aql->assign_editkey('general','username',trim($_POST['new_username']));
			} else {
				$aql->assign_append('general','username',trim($_POST['new_username']));
			}

			if(isset($web_server_array['general']['password'])) {
				$aql->assign_editkey('general','password',trim($_POST['new_password']));
			} else {
				$aql->assign_append('general','password',trim($_POST['new_password']));
			}
	...
	exec("php -r \"include_once('/www/cgi-bin/inc/wrcfg.inc');save_webserver_to_lighttpd();\" > /dev/null 2>&1");
