		...
		if($cluster_info['mode'] == 'master') {
			for($b=2; $b<=$__BRD_SUM__; $b++) {
				if(isset($cluster_info[$__BRD_HEAD__.$b.'_ip']) && $cluster_info[$__BRD_HEAD__.$b.'_ip'] != '') {
					for($i=1; $i<=$__GSM_SUM__; $i++) {
						$index = $__BRD_HEAD__.$b.'-'.$__GSM_HEAD__.$i;
						$input = $__BRD_HEAD__.$b.'-'.$__GSM_HEAD__.$i."-in";
						$output = $__BRD_HEAD__.$b.'-'.$__GSM_HEAD__.$i."-out";
						if(isset($_POST[$input])) {
							$invalue = trim($_POST[$input]);
							if($invalue != '') {
								$res = splitcmd($invalue);
								...
								$ret[$index]['output'] = $output;
								$ret[$index]['invalue'] = $invalue;
								$ret[$index]['ip'] = $cluster_info[$__BRD_HEAD__.$b.'_ip'];
								$ret[$index]['type'] = 'slave';
							}
						}
					}
				}
...
function send_new($ary,$type){
	...
	while (($handle_task) && ($timeup==false)){
		...
		if ($pop_str<>""){
			...
			//echo "==============================="."<br>";
			//echo "<pre>";
			//print_r($redis_client->lrange("app.asterisk.php.feedbacklist",0,-1));  //Array ( [0] => 222 [1] => 111 );
			//echo "</pre>";
			//echo "==============================="."<br>";
