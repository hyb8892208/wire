...
if($_POST && isset($_POST['send']) && $_POST['send']=='Save') {
	...
	if($product_type < 4){
		exec("uci set network.wan.proto=$_type");
		exec("uci set network.wan.ipaddr=$_ipaddr");
		exec("uci set network.wan.netmask=$_netmask");
		exec("uci set network.wan.gateway=$_gateway");
	...
	if($product_type < 4){
		wait_apply("exec", "/etc/init.d/network start > /dev/null 2>&1 &");
	}else{
...
if($product_type < 4){
	$cf_type = trim(`uci get network.wan.proto`);
	$cf_ip = trim(`uci get network.wan.ipaddr`);
	$cf_mac = trim(`uci get network.wan.macaddr`);
	$cf_mask = trim(`uci get network.wan.netmask`);
	$cf_gw = trim(`uci get network.wan.gateway`);
...
$factory_ip = str_replace('X',$slot_num,$__FACTORY_WAN_IP__);
