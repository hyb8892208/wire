...
<form id="manform" enctype="multipart/form-data" action="<?php echo get_self(); ?>" method="post">
	<div id="tab">
		<li class="tb1">&nbsp;</li>
		<li class="tbg"><?php echo language('Module Update');?></li>
		<li class="tb2">&nbsp;</li>
	</div>
		...
		<tr>
			<th>
				<div class="helptooltips">
					...
					<span class="showhelp">
					<?php echo language('Port help@module update','Select the module port that needs to be updated.');?>
					</span>
				</div>
			</th>
			<td>
		...
		<tr>
			<th>
				<div class="helptooltips">
					...
					<span class="showhelp">
						<?php echo language('Module Update file help');?>
					</span>
				</div>
			</th>
			<td>
...
function module_update($module_sel){
	if(isset($_FILES['update_module_file']['error']) && $_FILES['update_module_file']['error'] == 0){
		...
		if($product_type < 4){
			$module_file_path = "/tmp/module/";
		}else{
			$module_file_path = "/data/module/";
		}
		
		if(!file_exists($module_file_path)) exec("mkdir $module_file_path");
		$store_file = $module_file_path.$_FILES['update_module_file']['name'];
		...
		if(strstr($_FILES['update_module_file']['name'], 'M35')){
			...
			if($product_type < 4){
				exec("mount -o size=200M -o nr_inodes=10000 -o noatime,nodiratime -o remount /tmp");
			}
			
			exec("unzip $store_file -d $module_file_path");
			exec("rm -rf $store_file");
			exec("/my_tools/upgrade_module_ec20.sh $module_sel[0] $module_file_path");
			...
			exec("unzip $store_file -d $module_file_path");
			exec("rm -rf $store_file");
			exec("/my_tools/upgrade_module_uc15.sh $module_sel[0] $module_file_path");
		}
...
<!-- MCU update begin -->
	<div id="tab">
		<li class="tb1">&nbsp;</li>
		<li class="tbg"><?php echo language('MCU Update');?></li>
		<li class="tb2">&nbsp;</li>
	</div>
		...
		<tr>
			<th>
				<div class="helptooltips">
					<?php echo language('Port');?>:
					<span class="showhelp">
					<?php echo language('Port help@MCU update','Select the MCU port that needs to be updated.');?>
					</span>
				</div>
			</th>
			<td>
		...
		<tr>
			<th>
				<div class="helptooltips">
					...
					<span class="showhelp">
						<?php echo language('MCU Update file help');?>
					</span>
				</div>
			</th>
			<td>
...
?>
	...
	function mcu_set_timeout(chn, type){//Calculation timeout
		timeout_flag = setTimeout(function(){
			if(mcu_runtime < 180){//time out
				clearTimeout(timeout_flag);
				mcu_runtime++;
				mcu_set_timeout(chn, type);
			}else{
				exit_upgrade_module_mcu();
			}
		},1000);
	}
	
	function exit_upgrade_module_mcu(){
		$.ajax({
			type: "GET",
			url: "ajax_server.php?type=mcu_exit_upgrade",
			success: function(data){
			}
		});
	}
	...
	function mcu_update(chn){
		...
		clearTimeout(timeout_flag);
		mcu_runtime = 0;
		mcu_set_timeout(chn, '');//Calculation timeout
		...
		$.ajax({
			...
			success: function(data){
				if(data.indexOf('success') != -1){
					...
					$(".err_res_"+module_sel[next]).html("<span style='color:green'><?php echo language('Update success')?>!</span>");
		...
		$.ajax({
			...
			success: function (data){
				if(data.indexOf('success') != -1){
					...
					$(".err_res_"+restart_sel[re_next]).html("<span style='color:green'><?php echo language('Update success')?>!</span>");
...
<!-- EMU update begin -->
	<div id="tab">
		<li class="tb1">&nbsp;</li>
		<li class="tbg"><?php echo language('EMU Update');?></li>
		<li class="tb2">&nbsp;</li>
	</div>
		...
		<tr>
			<th>
				<div class="helptooltips">
					...
					<span class="showhelp">
					<?php echo language('Select a EMU to Update help');?>
					</span>
				</div>
			</th>
			<td>
		...
		<tr>
			<th>
				<div class="helptooltips">
					...
					<span class="showhelp">
					<?php echo language('Baud Rate help');?>
					</span>
				</div>
			</th>
			<td>
		...
		<tr>
			<th>
				<div class="helptooltips">
					...
					<span class="showhelp">
					<?php echo language('EMU Update File help');?>
					</span>
				</div>
			</th>
			<td>
