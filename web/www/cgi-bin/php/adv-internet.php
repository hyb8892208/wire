...
<form enctype="multipart/form-data" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
	<div id="tab">
		<li class="tb1">&nbsp;</li>
		<li class="tbg"><?php echo language('Internet');?></li>
		<li class="tb2">&nbsp;</li>
	</div>
...
?>
	...
	<table width="100%" class="tshow">
		...
		<tr>
			...
			<!--<th width=""><?php echo language('Timing Reset');?></th>
			<th width=""><?php echo language('Reset Time');?></th>-->
...
<?php
	...
	for ($i = 0; $i <= $__GSM_SUM__; $i++) {
		...
		/*
		$reset_type = '';
		if(isset($res[$port]['reset_type'])){
			$reset_type = trim($res[$port]['reset_type']);
		}
		
		$day = 1;
		if(isset($res[$port]['day'])){
			$day = trim($res[$port]['day']);
		}
		
		$hour = 0;
		if(isset($res[$port]['hour'])){
			$hour = trim($res[$port]['hour']);
		}
		
		$minute = 0;
		if(isset($res[$port]['minute'])){
			$minute = trim($res[$port]['minute']);
		}
		*/
		$flow_total_size = '';
...
function save_all_internet(){
	...
	$write = '';
	if($product_type < 4){
		$save_reset_tools = '/my_tools/opvx-ppp.sh > /tmp/reboot.log';
	}else{
